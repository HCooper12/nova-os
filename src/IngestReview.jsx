  ...
  return (
    <div style={css("border:1px solid color-mix(in srgb, var(--nv-ink) 09%, transparent);border-radius:10px;overflow:hidden;background:rgba(0,0,0,.22)")}>
      <div style={css("display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid color-mix(in srgb, var(--nv-ink) 06%, transparent)")}>
        <span style={css(`font:500 8.5px 'IBM Plex Mono',monospace;letter-spacing:.1em;color:${KIND_COLOR[change.kind]};border:1px solid ${KIND_COLOR[change.kind]};border-radius:5px;padding:2px 7px;flex:none`)}>{change.kind.toUpperCase()}</span>
        <span style={css("font-size:12.5px;color:color-mix(in srgb, var(--nv-ink) 85%, transparent);word-break:break-all")}>{change.path}</span>
      </div>
      <div style={css("max-height:180px;overflow-y:auto;padding:12px 14px;font:400 11.5px/1.6 'IBM Plex Mono',monospace;color:color-mix(in srgb, var(--nv-ink) 70%, transparent);white-space:pre-wrap")}>{change.content}</div>
    </div>
  );
}
    ...
    <div role="dialog" aria-modal="true" aria-label="Review ingest changes" onClick={processing ? undefined : v.closeIngestReview} style={css("position:fixed;inset:0;background:rgba(8,5,12,.72);backdrop-filter:blur(6px);z-index:60;display:flex;align-items:center;justify-content:center;padding:40px;overflow-y:auto")}>
      <div onClick={v.stopClick} style={css("width:700px;max-width:94vw;max-height:88vh;overflow-y:auto;border:1px solid var(--nv-edge);border-radius:var(--nv-radius);background:var(--nv-glass2);backdrop-filter:blur(22px);box-shadow:0 40px 90px -30px rgba(0,0,0,.95),inset 0 1px 0 var(--nv-spec);animation:fadeUp .3s ease-out;padding:26px 28px")}>
        <div style={css("display:flex;justify-content:space-between;align-items:center")}>
          <span style={css("font:500 9.5px 'IBM Plex Mono',monospace;letter-spacing:.24em;color:var(--nv-gold)")}>INGEST · REVIEW</span>
          {!processing && !applying && (
            <Interactive as="span" onClick={v.closeIngestReview} base="cursor:pointer;font:500 11px 'IBM Plex Mono',monospace;color:color-mix(in srgb, var(--nv-ink) 50%, transparent);border:1px solid color-mix(in srgb, var(--nv-ink) 14%, transparent);border-radius:7px;padding:5px 10px" hoverStyle="color:var(--nv-ink)">ESC</Interactive>
          )}
        </div>
