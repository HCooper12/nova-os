...
export function FocusChip({ v }) {
  ...
  return (
    <div style={css(`display:flex;align-items:center;gap:10px;flex-wrap:wrap;border:1px solid color-mix(in srgb, var(--nv-cy) ${done ? '55' : '30'}%, transparent);border-radius:11px;padding:9px 14px;background:color-mix(in srgb, var(--nv-cy) 06%, transparent)`)}>
      <span style={css("font:var(--nv-micro-s);letter-spacing:var(--nv-micro-track-wide);color:var(--nv-cy)")}>{done ? 'FOCUS BLOCK DONE' : 'FOCUS'}</span>
      ...
      {done ? (
        <>
          <Interactive as="span" onClick={v.log} base="cursor:pointer;font:var(--nv-micro-m);letter-spacing:var(--nv-micro-track);padding:6px 12px;border-radius:7px;background:var(--nv-cy);color:var(--nv-on-acc)" hoverStyle="filter:brightness(1.08)">JOURNAL IT</Interactive>
          <Interactive as="span" onClick={v.dismiss} base="cursor:pointer;font:var(--nv-micro-m);color:var(--nv-ink40)" hoverStyle="color:var(--nv-ink)">dismiss</Interactive>
        </>
      ) : (
        <Interactive as="span" onClick={v.dismiss} base="cursor:pointer;font:var(--nv-micro-m);color:var(--nv-ink40);margin-left:auto" hoverStyle="color:var(--nv-warn)">abandon</Interactive>
      )}
    </div>
  );
}
