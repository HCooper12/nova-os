...
export function MobileChrome({ v }) {
  ...
  return (
    <>
      <div style={css("position:fixed;top:0;left:0;right:0;z-index:70;display:flex;align-items:center;gap:10px;padding:calc(10px + env(safe-area-inset-top)) max(16px, env(safe-area-inset-right)) 10px max(16px, env(safe-area-inset-left));background:var(--nv-glass2);backdrop-filter:blur(14px);border-bottom:1px solid var(--nv-edge)")}>
      ...
      <div style={css("position:fixed;bottom:0;left:0;right:0;z-index:70;background:var(--nv-glass2);backdrop-filter:blur(14px);border-top:1px solid var(--nv-edge);padding-bottom:env(safe-area-inset-bottom)")}>
        <div ref={scrollRef} className="nv-tabscroll" style={{ display: 'flex', gap: '2px', overflowX: 'auto', padding: '6px max(8px, env(safe-area-inset-left)) 6px max(8px, env(safe-area-inset-right))', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {v.tabs.map((t) => (
