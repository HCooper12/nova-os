    ...
    <div role="dialog" aria-modal="true" aria-label={weightMode ? 'Weight history' : 'Step history'} onClick={v.close} style={css("position:fixed;inset:0;background:rgba(8,5,12,.82);backdrop-filter:blur(6px);z-index:80;display:flex;align-items:center;justify-content:center;padding:18px;overflow-y:auto")}>
      <div onClick={(e) => e.stopPropagation()} style={css("width:560px;max-width:96vw;max-height:92vh;overflow-y:auto;border:1px solid var(--nv-edge);border-radius:var(--nv-radius);background:var(--nv-glass2);backdrop-filter:blur(22px);box-shadow:0 40px 90px -30px rgba(0,0,0,.95);padding:22px 24px")}>
        <div style={css("display:flex;justify-content:space-between;align-items:center")}>
          <span style={css("font:500 9.5px 'IBM Plex Mono',monospace;letter-spacing:.22em;color:var(--nv-mg)")}>{weightMode ? 'WEIGHT · LAST 14 DAYS' : 'STEPS · LAST 7 DAYS'}</span>
        ...
        {weightMode ? (
          <>
            <div style={css("margin-top:14px;text-align:center")}>
              {v.weightHero ? (
                <>
                  <div style={{ font: "700 46px 'Rajdhani',sans-serif", lineHeight: 1, fontVariantNumeric: 'tabular-nums', color: 'var(--nv-gold)' }}>
                    {v.weightHero.kg}<small style={{ fontSize: '18px', fontWeight: 600, color: 'var(--nv-ink40)', marginLeft: '4px' }}>KG</small>
                  </div>
                  <div style={css("margin-top:4px;font:500 11.5px 'IBM Plex Mono',monospace;color:var(--nv-ink60)")}>{v.weightHero.sub}</div>
                </>
              ) : (
                <>
                  <div style={{ font: "700 46px 'Rajdhani',sans-serif", lineHeight: 1, color: 'var(--nv-ink40)' }}>—</div>
                  <div style={css("margin-top:4px;font:500 11.5px 'IBM Plex Mono',monospace;color:var(--nv-ink60)")}>no readings yet — tap a day below to log one</div>
                </>
              )}
            </div>
            <div style={{ marginTop: '14px', borderTop: '1px solid var(--nv-edge)' }}>
              {v.weightDays.map((d, i) => (
                <Interactive key={d.date} onClick={d.startEdit}
                  ...
                  <span style={{ font: "500 11px 'IBM Plex Mono',monospace", color: d.editing ? 'var(--nv-cy)' : d.isToday ? 'var(--nv-cy)' : 'var(--nv-ink60)', width: '108px', flex: 'none' }}>{d.full}{d.isToday ? ' · today' : ''}</span>
                  <span style={{ marginLeft: 'auto', font: "600 13px 'Rajdhani',sans-serif", fontVariantNumeric: 'tabular-nums', color: d.hasData ? 'var(--nv-gold)' : 'var(--nv-ink40)' }}>
