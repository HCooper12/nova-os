...
export function StepsHistory({ v }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={weightMode ? 'Weight history' : 'Step history'} onClick={v.close} style={css("position:fixed;inset:0;background:rgba(8,5,12,.82);backdrop-filter:blur(6px);animation:fadeIn var(--nv-dur-base) var(--nv-ease);z-index:80;display:flex;align-items:center;justify-content:center;padding:18px;overflow-y:auto")}>
      {/* the SAME name the tile had, so the tile expands into this panel */}
      <div onClick={(e) => e.stopPropagation()} style={{
        ...css("width:560px;max-width:96vw;max-height:92vh;overflow-y:auto;border:1px solid var(--nv-edge);border-radius:var(--nv-radius);background:var(--nv-glass2);backdrop-filter:blur(22px);box-shadow:0 40px 90px -30px rgba(0,0,0,.95);padding:22px 24px;animation:fadeUp var(--nv-dur-base) var(--nv-ease)"),
        ...(v.vtName ? { viewTransitionName: v.vtName } : {}),
      }}>
        <div style={css("display:flex;justify-content:space-between;align-items:center")}>
          <span style={css("font:var(--nv-micro-m);letter-spacing:var(--nv-micro-track-wide);color:var(--nv-mg)")}>{weightMode ? 'WEIGHT · LAST 14 DAYS' : 'STEPS · LAST 7 DAYS'}</span>
        ...
        {weightMode ? (
            ...
            <div style={css("margin-top:14px;text-align:center")}>
              {v.weightHero ? (
                <>
                  ...
                  <div style={css("margin-top:4px;font:var(--nv-micro-l);color:var(--nv-ink60)")}>{v.weightHero.sub}</div>
                </>
              ) : (
                <>
                  <div style={{ font: "700 46px var(--nv-font-ui)", lineHeight: 1, color: 'var(--nv-ink40)' }}>—</div>
                  <div style={css("margin-top:4px;font:var(--nv-micro-l);color:var(--nv-ink60)")}>no readings yet — tap a day below to log one</div>
                </>
              )}
            </div>
            <div style={{ marginTop: '14px', borderTop: '1px solid var(--nv-edge)' }}>
              {v.weightDays.map((d, i) => (
                <Interactive key={d.date} onClick={d.startEdit}
                  ...
                  <span style={{ font: 'var(--nv-micro-l)', color: d.editing ? 'var(--nv-cy)' : d.isToday ? 'var(--nv-cy)' : 'var(--nv-ink60)', width: '108px', flex: 'none' }}>{d.full}{d.isToday ? ' · today' : ''}</span>
          ...
          <>
            <div style={css("margin-top:14px;text-align:center")}>
              ...
              <div style={css("margin-top:4px;font:var(--nv-micro-l);color:var(--nv-ink60)")}>
            ...
            <div style={{ marginTop: '18px', position: 'relative', display: 'flex', alignItems: 'flex-end', gap: '6px', height: '176px', borderBottom: '1px solid var(--nv-edge)' }}>
              ...
              {v.days.map((d) => (
                <Interactive key={d.date} onClick={d.startEdit} base={{ cursor: 'pointer', flex: 1, minWidth: 0, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: '4px' }} hoverStyle={{ opacity: 0.85 }}>
                  <span style={{ font: 'var(--nv-micro-s)', fontVariantNumeric: 'tabular-nums', color: d.hasData ? barColor(d) : 'var(--nv-ink40)' }}>{d.hasData ? d.steps.toLocaleString() : '—'}</span>
                  <div style={{
            ...
            <div style={{ display: 'flex', gap: '6px', marginTop: '6px' }}>
              {v.days.map((d) => (
                <span key={d.date} style={{ flex: 1, minWidth: 0, textAlign: 'center', font: 'var(--nv-micro-m)', color: d.isToday ? 'var(--nv-cy)' : 'var(--nv-ink40)' }}>{d.label}</span>
              ))}
            </div>

            <div style={css("margin-top:14px;text-align:center;font:var(--nv-micro-l);color:var(--nv-ink60)")}>
        ...
        {v.editDate && (
          <div style={css("margin-top:16px;border-top:1px solid var(--nv-edge);padding-top:14px;display:flex;gap:8px;align-items:center;flex-wrap:wrap")}>
            ...
            <Interactive as="span" onClick={v.saveEdit} base="cursor:pointer;font:var(--nv-micro-m);letter-spacing:var(--nv-micro-track);padding:9px 15px;border-radius:8px;background:var(--nv-cy);color:var(--nv-on-acc)" hoverStyle="filter:brightness(1.08)">SAVE</Interactive>
            <Interactive as="span" onClick={v.cancelEdit} base="cursor:pointer;font:var(--nv-micro-m);color:var(--nv-ink40)" hoverStyle="color:var(--nv-ink)">cancel</Interactive>
          </div>
        )}
