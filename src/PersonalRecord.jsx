    ...
    <div style={css('position:relative;width:158px;height:158px;margin:0 auto')}>
      ...
      <div style={css('position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1px')}>
        <CountUp
          ...
          style={{ font: `700 34px var(--nv-font-ui)`, letterSpacing: 'var(--nv-display-track)', color: 'var(--nv-ink)', lineHeight: 1 }}
        />
        <span style={css('font:var(--nv-micro-s);letter-spacing:var(--nv-micro-track);color:var(--nv-ink40)')}>{unit}</span>
      </div>
    </div>
  );
}
...
export function PersonalRecord({ records, onClose }) {
  ...
  const unit = lead.kind === 'weight' ? 'KG' : 'E1RM · KG';
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Personal record" onClick={exit.close} ref={exit.scrimRef}
      ...
      <div ref={exit.panelRef} onClick={(e) => e.stopPropagation()} className="nv-liquid"
        style={css('width:min(340px,100%);padding:26px 22px 22px;text-align:center;animation:prRise var(--nv-dur-slow) var(--nv-ease) both')}>
        ...
        {/* The sentence carries the magnitude, because the ring deliberately
            does not. "First on record" when there is nothing to have passed —
            an absent previous is said, never invented. */}
        <div style={css('margin-top:5px;font:500 13.5px var(--nv-font-ui);color:var(--nv-ink60)')}>
          {prev != null
            ? <>past {prev}{delta > 0 ? <span style={css('color:var(--nv-gold)')}>{'  ·  +'}{deltaText} kg</span> : null}</>
            : 'first on record'}
          {lead.kind === 'weight' && lead.reps ? <span style={css('color:var(--nv-ink40)')}>{'  ·  '}{lead.reps} reps</span> : null}
        </div>
            ...
            {list.slice(1).map((p, i) => (
              <div key={i} style={css('display:flex;justify-content:space-between;gap:12px;font:500 13px var(--nv-font-ui);color:var(--nv-ink60)')}>
                ...
                <span style={css('flex:none;color:var(--nv-ink);font-variant-numeric:tabular-nums')}>
                  {p.kind === 'weight' ? `${p.value} kg × ${p.reps}` : `${p.value} kg`}
                </span>
              </div>
            ))}
          </div>
