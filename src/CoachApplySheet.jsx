  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Confirm the Coach's change" onClick={c.busy ? undefined : c.cancel}
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={css(`font:600 9px ${M};letter-spacing:.22em;color:var(--nv-gold)`)}>◆ COACH · CONFIRM THE CHANGE</div>
        ...
        <div style={css('margin-top:14px;display:flex;gap:10px;justify-content:flex-end')}>
          <Interactive as="span" onClick={c.busy ? undefined : c.cancel}
            base={`cursor:pointer;font:600 10.5px ${M};letter-spacing:.08em;padding:10px 16px;border-radius:9px;border:1px solid color-mix(in srgb, var(--nv-ink) 16%, transparent);color:color-mix(in srgb, var(--nv-ink) 60%, transparent);opacity:${c.busy ? 0.5 : 1}`}
            hoverStyle="background:rgba(255,255,255,.05)">CANCEL</Interactive>
          <Interactive as="span" onClick={c.busy ? undefined : c.confirm}
            base={{ cursor: c.busy ? 'default' : 'pointer', font: `600 10.5px ${M}`, letterSpacing: '.08em', padding: '10px 20px', borderRadius: '9px', background: 'var(--nv-gold)', color: '#1a1322', opacity: c.busy ? 0.6 : 1 }}
