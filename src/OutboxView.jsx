...
export function OutboxView({ v }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Outbox" onClick={exit.close} ref={exit.scrimRef} style={css("position:fixed;inset:0;background:rgba(8,5,12,.78);backdrop-filter:blur(6px);animation:fadeIn var(--nv-dur-base) var(--nv-ease);z-index:85;display:flex;align-items:center;justify-content:center;padding:18px")}>
      <div ref={exit.panelRef} onClick={(e) => e.stopPropagation()} style={css("width:520px;max-width:96vw;max-height:88vh;overflow-y:auto;border:1px solid var(--nv-edge);border-radius:var(--nv-radius);background:var(--nv-glass2);backdrop-filter:blur(22px);box-shadow:0 40px 90px -30px rgba(0,0,0,.9);padding:20px 22px;animation:fadeUp var(--nv-dur-base) var(--nv-ease)")}>
        ...
        <div style={css("margin-top:12px;display:flex;flex-direction:column")}>
          {v.items.map((it, i) => (
            <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 2px', borderTop: i === 0 ? 'none' : '1px solid color-mix(in srgb, var(--nv-ink) 07%, transparent)' }}>
              <span style={{ flex: 'none', font: 'var(--nv-micro-s)', letterSpacing: '.1em', padding: '3px 9px', borderRadius: '6px', color: it.failed ? 'var(--nv-warn)' : 'var(--nv-gold)', border: `1px solid color-mix(in srgb, ${it.failed ? 'var(--nv-warn)' : 'var(--nv-gold)'} 40%, transparent)` }}>{KIND_LABEL[it.kind] || it.kind}</span>
              <span style={{ minWidth: 0, flex: 1 }}>
                ...
                <span style={{ display: 'block', marginTop: '1px', font: 'var(--nv-micro-m)', color: it.failed ? 'var(--nv-warn)' : 'var(--nv-ink40)' }}>
                  {it.failed ? `rejected: ${it.error}` : `queued ${it.when}`}
                </span>
              </span>
              {it.failed && (
                <Interactive as="span" onClick={it.retry} base="cursor:pointer;flex:none;font:var(--nv-micro-s);letter-spacing:var(--nv-micro-track);padding:5px 10px;border-radius:6px;border:1px solid color-mix(in srgb, var(--nv-cy) 40%, transparent);color:var(--nv-cy)" hoverStyle="background:color-mix(in srgb, var(--nv-cy) 08%, transparent)">RETRY</Interactive>
              )}
              <Interactive as="span" onClick={it.discard} base="cursor:pointer;flex:none;font:var(--nv-micro-m);color:color-mix(in srgb, var(--nv-ink) 38%, transparent);padding:4px" hoverStyle="color:var(--nv-warn)">discard</Interactive>
            </div>
          ))}
          {v.items.length === 0 && (
