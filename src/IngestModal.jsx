    ...
    <div role="dialog" aria-modal="true" aria-label="Ingest a transcript" onClick={v.closeIngestModal} style={css("position:fixed;inset:0;background:rgba(8,5,12,.72);backdrop-filter:blur(6px);z-index:60;display:flex;align-items:center;justify-content:center;padding:40px;overflow-y:auto")}>
      <div onClick={v.stopClick} style={css("width:640px;max-width:94vw;max-height:88vh;overflow-y:auto;border:1px solid rgba(216,181,115,.28);border-radius:18px;background:linear-gradient(180deg,#221a2c,#16101e);box-shadow:0 40px 90px -30px rgba(0,0,0,.95),inset 0 1px 0 rgba(255,255,255,.07);animation:fadeUp .3s ease-out;padding:26px 28px")}>
        <div style={css("display:flex;justify-content:space-between;align-items:center")}>
          <span style={css("font:500 9.5px 'JetBrains Mono',monospace;letter-spacing:.24em;color:#d8b573")}>ADD · NEW CONTENT</span>
        ...
        <div style={css("margin-top:12px;font:500 9.5px 'JetBrains Mono',monospace;letter-spacing:.2em;color:rgba(236,229,218,.45)")}>SOURCE URL (OPTIONAL — SKIP FOR YOUR OWN NOTES)</div>
        <Interactive
          as="input"
        ...
        <div style={css("margin-top:14px;display:flex;align-items:center;gap:12px;flex-wrap:wrap")}>
          ...
          <div style={css("margin-left:auto;display:flex;gap:10px")}>
            ...
            <Interactive as="span" onClick={v.submitIngest} base="cursor:pointer;font-size:12.5px;font-weight:500;padding:9px 18px;border-radius:8px;background:#d8b573;color:#1a1322" hoverStyle="background:#e6c98f">Add to vault</Interactive>
          </div>
        </div>
      </div>
    </div>
