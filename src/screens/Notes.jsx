    ...
    <div style={v.wrapNotes} data-screen-label="Notes">
      <div style={css("display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px")}>
        <div style={css("display:flex;align-items:center;gap:14px")}>
          <span style={css("font:500 11px 'IBM Plex Mono',monospace;letter-spacing:.14em;color:var(--nv-acc)")}>VIII.</span>
          <span style={css("width:50px;height:1px;background:linear-gradient(90deg,var(--nv-acc-border),transparent)")}></span>
          <span style={css("font:500 10px 'IBM Plex Mono',monospace;letter-spacing:.32em;color:color-mix(in srgb, var(--nv-ink) 55%, transparent)")}>VAULT · NOTES</span>
        </div>
        <span style={css("font:400 10px 'IBM Plex Mono',monospace;letter-spacing:.12em;color:color-mix(in srgb, var(--nv-ink) 45%, transparent)")}>{v.notesHeaderLabel}</span>
      </div>
      <div style={v.gridNotes}>
        <div style={v.noteListCard}>
          <div style={css("padding:14px 14px 10px")}>
            <Interactive
              ...
              base="width:100%;box-sizing:border-box;background:rgba(0,0,0,.32);border:1px solid color-mix(in srgb, var(--nv-ink) 12%, transparent);border-radius:9px;padding:9px 13px;color:var(--nv-ink);font-size:12.5px;font-family:'Rajdhani',sans-serif;outline:none"
          ...
          <div style={css("flex:1;overflow-y:auto;padding:0 8px 10px;display:flex;flex-direction:column;gap:2px")}>
            {v.noteList.map((n, i) => (
              <Interactive key={i} onClick={n.select} base={n.style} hoverStyle="background:rgba(255,255,255,.05)">
                ...
                <div style={css("margin-top:3px;font:400 10px 'IBM Plex Mono',monospace;color:color-mix(in srgb, var(--nv-ink) 40%, transparent)")}>{n.date}</div>
              </Interactive>
            ))}
          </div>
        </div>
        <div style={css("border:1px solid var(--nv-edge);border-radius:var(--nv-radius);padding:26px 32px;background:var(--nv-glass);box-shadow:inset 0 1px 0 var(--nv-spec);overflow-y:auto")}>
          <div style={css(`font:500 9px 'IBM Plex Mono',monospace;letter-spacing:.22em;color:${v.openNoteTypeColor}`)}>{v.openNoteType}</div>
          <h2 style={css("margin:10px 0 0;font:400 32px/1.15 'Instrument Serif',serif")}>{v.openNoteTitle}</h2>
          <div style={css("margin-top:8px;font:400 10.5px 'IBM Plex Mono',monospace;color:color-mix(in srgb, var(--nv-ink) 40%, transparent)")}>{v.openNoteMeta}</div>
          {v.openNoteUrl && (
          ...
          {v.reviewShowReflect && (
            <div style={css("margin-top:24px;max-width:640px;padding:16px 18px;border-radius:12px;border:1px solid color-mix(in srgb, var(--nv-vi) 35%, transparent);background:color-mix(in srgb, var(--nv-vi) 06%, transparent)")}>
              <div style={css("display:flex;justify-content:space-between;align-items:center")}>
                <span style={css("font:500 9.5px 'IBM Plex Mono',monospace;letter-spacing:.24em;color:var(--nv-vi)")}>TODAY'S REVIEW · SUMMARY</span>
              ...
              <div style={css("margin-top:10px;font:400 15px/1.6 'Instrument Serif',serif;color:color-mix(in srgb, var(--nv-ink) 90%, transparent);text-wrap:pretty")}>{v.reviewConcept}</div>
              {v.reviewReflectOpen && (
                <div style={css("margin-top:16px;padding-top:16px;border-top:1px solid color-mix(in srgb, var(--nv-vi) 15%, transparent)")}>
                  <div style={css("display:flex;justify-content:flex-end")}>
                    <Interactive
                      ...
                      base={{ cursor: 'pointer', font: "500 9.5px 'IBM Plex Mono',monospace", padding: '5px 10px', borderRadius: '6px', color: '#cbb6f2', opacity: v.reviewReflectPromptBusy ? .6 : 1 }}
                  ...
                  <textarea
                    ...
                    style={css("width:100%;box-sizing:border-box;height:110px;resize:vertical;background:rgba(0,0,0,.28);border:1px solid color-mix(in srgb, var(--nv-ink) 12%, transparent);border-radius:8px;padding:11px 14px;color:var(--nv-ink);font-size:13.5px;font-family:'Rajdhani',sans-serif;line-height:1.6;outline:none")}
                  />
                  <div style={css("margin-top:10px;display:flex;justify-content:flex-end")}>
                    <Interactive
                      ...
                      base={{ cursor: 'pointer', font: "500 10.5px 'IBM Plex Mono',monospace", padding: '7px 14px', borderRadius: '7px', background: 'var(--nv-vi)', color: '#1a1322', opacity: v.reviewReflectBusy ? .6 : 1 }}
          ...
          <div style={css("margin-top:26px;padding-top:16px;border-top:1px solid color-mix(in srgb, var(--nv-ink) 08%, transparent)")}>
            <div style={css("font:500 9px 'IBM Plex Mono',monospace;letter-spacing:.22em;color:color-mix(in srgb, var(--nv-ink) 40%, transparent)")}>LINKED IN GALAXY</div>
