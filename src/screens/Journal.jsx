    ...
    <div style={v.wrapJournal} data-screen-label="Journal">
      <div style={css("display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px")}>
        <div style={css("display:flex;align-items:center;gap:14px")}>
          ...
          <span style={css("font:500 10px 'IBM Plex Mono',monospace;letter-spacing:.32em;color:color-mix(in srgb, var(--nv-ink) 55%, transparent)")}>VAULT · JOURNAL</span>
        </div>
        <span style={css("font:400 10px 'IBM Plex Mono',monospace;letter-spacing:.12em;color:color-mix(in srgb, var(--nv-ink) 45%, transparent)")}>{v.journalHeaderLabel}</span>
      </div>
      <h1 style={css("margin:18px 0 0;font:700 30px/1.1 'Rajdhani',sans-serif;letter-spacing:.02em")}>Write it <span style={css("font:italic 400 27px 'Instrument Serif',serif;color:var(--nv-gold)")}>down.</span></h1>
      ...
      <div style={css("margin-top:20px;border:1px solid color-mix(in srgb, var(--nv-vi) 25%, transparent);border-radius:14px;padding:18px 20px;background:linear-gradient(180deg,color-mix(in srgb, var(--nv-vi) 06%, transparent),color-mix(in srgb, var(--nv-vi) 01%, transparent))")}>
        <div style={css("display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px")}>
          <span style={css("font:500 9.5px 'IBM Plex Mono',monospace;letter-spacing:.2em;color:var(--nv-vi)")}>NEW ENTRY</span>
          <Interactive
            as="span"
            onClick={v.journalPromptBusy ? undefined : v.generateJournalPrompt}
            base={{ cursor: 'pointer', font: "500 10px 'IBM Plex Mono',monospace", padding: '7px 13px', borderRadius: '7px', border: '1px solid color-mix(in srgb, var(--nv-vi) 40%, transparent)', color: '#cbb6f2', background: 'color-mix(in srgb, var(--nv-vi) 08%, transparent)', opacity: v.journalPromptBusy ? .6 : 1 }}
        ...
        <textarea
          ...
          style={css("margin-top:14px;width:100%;box-sizing:border-box;height:120px;resize:vertical;background:rgba(0,0,0,.3);border:1px solid color-mix(in srgb, var(--nv-ink) 12%, transparent);border-radius:9px;padding:12px 14px;color:var(--nv-ink);font-size:13.5px;font-family:'Rajdhani',sans-serif;line-height:1.6;outline:none")}
        />
        {v.journalSaveError && (
        ...
        <div style={css("margin-top:10px;display:flex;justify-content:flex-end")}>
          <Interactive
            ...
            base={{ cursor: 'pointer', font: "500 11px 'IBM Plex Mono',monospace", padding: '9px 18px', borderRadius: '8px', background: 'var(--nv-gold)', color: '#1a1322', opacity: v.journalSaveBusy ? .6 : 1 }}
          ...
          {v.journalDays.map((d) => (
            <div key={d.date} style={css("border:1px solid color-mix(in srgb, var(--nv-ink) 09%, transparent);border-radius:12px;padding:14px 18px;background:rgba(255,255,255,.02)")}>
              <Interactive as="div" onClick={d.toggle} base="cursor:pointer;display:flex;justify-content:space-between;align-items:baseline;gap:10px" hoverStyle={{}}>
                ...
                <span style={css("font:400 10px 'IBM Plex Mono',monospace;color:color-mix(in srgb, var(--nv-ink) 40%, transparent);text-align:right;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin:0 10px")}>{d.open ? '' : d.preview}</span>
                <span style={css("font:400 10px 'IBM Plex Mono',monospace;color:color-mix(in srgb, var(--nv-ink) 40%, transparent)")}>{d.count} {d.count === 1 ? 'entry' : 'entries'} {d.open ? '▲' : '▼'}</span>
              </Interactive>
              {d.open && (
