...
const only = opt('only');
const mode = opt('film', 'cycle');
const T0 = Number(opt('t0', 0)), T1 = Number(opt('t1', 2)), N = Number(opt('n', 12)), yaw = Number(opt('yaw', 0.45));
const shots = []; let n = 0;
...
if (mode === 'cycle') {
  ...
  for (let k = 0; k < N; k++) { const t = T0 + (T1 - T0) * k / N; await ev(`__agentSheet.still({t:${t},yaw:${yaw},elev:0.12})`); await snap('t' + t.toFixed(2)); }
} else {
  // the switch: wait settled, then work, stepping the clock at 15fps
  await ev(`__agentSheet.pose(${idx},'wait')`);
  await ev(`__agentSheet.still({t:5,yaw:${yaw},elev:0.12})`); await snap('wait');
  await ev(`__agentSheet.pose(${idx},'work')`);
  for (let k = 1; k < N; k++) { await ev(`__agentSheet.still({t:${5 + k / 15},yaw:${yaw},elev:0.12,snap:false})`); await snap('k' + k); }
}
while (shots.length % 6) { await snap('pad'); }
console.log(logs.filter(l => !/^\[fit\]/.test(l)).join('\n'));
ws.close(); await cleanup();
execFileSync('ffmpeg', ['-y', '-loglevel', 'error', '-i', path.join(outDir, 'f_%02d.png'), '-vf', `scale=330:-1,tile=6x${Math.ceil(shots.length/6)}`, '-frames:v', '1', path.join(outDir, 'sheet.png')]);
