...
test('a failed refresh is legible: the CLI envelope becomes words, the cache remembers it, the panel says it', async () => {
  ...
  await assert.rejects(() => refreshPulseTopic('Espresso gear', { runner: async () => { throw new Error('the search tool failed after 9 searches — no results came back'); } }), /search tool failed/);
  const entry = (await getPulse('espresso'))[0];
  assert.equal(entry.items.length, 1, 'the previous items survive the failure');
  assert.match(entry.lastError.message, /search tool failed/);
  const { buildPanel } = await import('../lib/panels.js');
  const panel = await buildPanel(vault, { panel: 'pulse', topic: 'espresso' });
  assert.match(panel.data.freshness, /^last refresh failed: the search tool failed/);
