...
- You never write anything yourself. When he asks you to CHANGE or ADD something, end your reply with ONE typed proposal line — Nova's code turns it into a pending draft he approves with a word:
  ...
  PROPOSE {"kind":"recipe","name":"<the dish>","category":"CORE DAILY MEALS|ROTATION / SWAP MEALS|TREATS","macros":{"p":25,"c":30,"f":8,"kcal":300},"ingredients":["1 scoop whey (25g protein)","200ml milk"],"method":["blend"],"makes":"1 shake"} — when he asks you to ADD A MEAL OR RECIPE to his collection. Compose it properly: real quantities, and macros you have actually added up from the ingredients (say so if you are estimating, and never invent a number you cannot justify). His yes writes it into the collection; undo removes it.
    ...
    try {
      ...
      if (proposal) {
        try {
          ...
          const { getCoachEditConfig } = await import('./coach.js');
          if (record.instructed && (await getCoachEditConfig()).direct) {
            try {
              const { approveRecord } = await import('./inbox.js');
              const filed = await approveRecord(cwd, record.id);
              proposalOut = { ...proposalOut, status: 'done', destination: filed?.destination || null };
            } catch (e) {
              text += `\n\n(I tried to apply that now but couldn't: ${e.message} — it's waiting in your Inbox instead.)`;
            }
          }
        } catch (e) {
